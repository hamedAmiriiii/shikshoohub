...
export default function SimpleBottomNavigationAtelier() {
  ...
  function getNavigationValue(path: string) {
    if (path.includes('/shikshoo/admin/product/create')) return 0; // ثبت کالا
    if (path.includes('/shikshoo/admin/purchas')) return 1; // خریدها
    if (path.includes('/shikshoo/admin/product')) return 2; // کالاها
    if (path === '/shikshoo/admin' || path.includes('/shikshoo/admin/') && !path.includes('/shikshoo/admin/product') && !path.includes('/shikshoo/admin/purchas')) return 3; // خانه
  ...
  const handleNavChange = (event: any, newValue: number) => {
    ...
    if (newValue === 0) {
      router.push("/shikshoo/admin/product/create")

     
    } else if (newValue === 1) {
      // setPurchasesSheetOpen(true);
      // setProductsSheetOpen(false);
      router.push("/shikshoo/admin/purchas")

      }else if (newValue === 2) {
        // setProductsSheetOpen(true);
        // setPurchasesSheetOpen(false);
        router.push("/shikshoo/admin/product")
    }else if (newValue === 3) {
