...
export default function Page() {
  ...
  const confirm = () => {
    ...
    let token = tokenCode()
    apiRequestError("Post", {}, data, "/api/admin/garden", true, true, token).then((res) => {
      setloading(false)
      if (res.hasError) {
      ...
      toast.success("باغ با موفقیت ثبت شد")
    setTimeout(() => {
      router.push("/main/bagh")
    }, 1200);  
...

      {
        ...
        <Box sx={{ textAlign: "center", }}>
          ...
          <Box mt={1} sx={{ width: "100%", color: "#fff", marginTop: "20px", marginRight: "20px" }}>
            <Typography textAlign="right">تلفن مالک باغ :</Typography>
            <PhoneNumberInput defaultValue={phon} onChange={onChangePhone}
              name="phoneNumber" />
            <TextInput value={full_name} label="نام باغ" onChange={(e) => setfull_name(e)} name="fulname" />
          ...
          <Box sx={{ marginTop: "20px", height: "20vh" }}>
            ...
            <Button onClick={() => router.push("/main/bagh")} sx={{ marginTop: "15px", borderRadius: "25px", bgcolor: "#ff9100", color: "#fff", width: "30%", height: "60px", fontWeight: "600" }}
              variant="filled" >
              انصراف
            </Button>
          </Box>
