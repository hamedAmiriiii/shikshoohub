...
export default function ShopHomePage() {
  ...
  const fetchProducts = useCallback(async (page: number = 1, isInitial: boolean = false, search: string = '') => {
    try {
      ...
      let url = shopApi(`/api/product?page=${page}`);
      if (search) {
        url += `&search=${encodeURIComponent(search)}`;
      }
      
      const token = getCustomerToken() || tokenCode() || '';
      const res = await apiRequestError("Get", {}, {}, url, !!token, true, token);
  ...
  useEffect(() => {
    const fetchBestSelling = async () => {
      ...
      try {
        ...
        const res = await apiRequestError("Get", {}, {}, shopApi("/api/product/best-selling?limit=10"), !!token, true, token);
