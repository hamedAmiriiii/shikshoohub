...
export default function SimpleBottomNavigation() {
  ...
  function getNavigationValue(path: string) {
    if (path.includes('/home')) return 4;
    if (path.includes('/atelie')) return 3;
      ...
      <Paper sx={{width:"94%", position: 'fixed', bottom: 12, left: '3%', right: 0 ,borderRadius:'30px', height:"65"}} elevation={2}>
        ...
        <BottomNavigation
          ...
          onChange={(event: any, newValue: number) => {
            ...
            let link = newValue == 4 ? "/main/home" : newValue == 3 ? "/main/atelie" : newValue == 2 ? "/main/cameraman" : newValue == 1 ? "/main/talar" : newValue == 0 ? "/main/bagh"
              : "/"
            router.push(link)
setTimeout(() => {
  ...
  setValue(newValue);
        ...
        >
          ...
          <BottomNavigationAction label={translate("tasks")} icon={<AssignmentIcon />} />
